...
type BMResults struct {
	PubRuns   []*PubResults    `json:"publish_runs"`
	SubRuns   []*SubResults    `json:"subscribe_runs"`
	PubTotals *TotalPubResults `json:"publish_totals"`
	SubTotals *TotalSubResults `json:"receive_totals"`
}

// Start initiates the benchmarking process by starting the subscribe and publish operations.
...
// It returns the benchmark results and an error, if any.
func Start(broker string, topic string, qos int, size int, count int, clients int, quiet bool) (*BMResults, error) {
	...
	// SUBJOBDONE is a label for the loop that waits for all subscribe jobs to complete.
SUBJOBDONE:
	for {
		select {
		case ret := <-subDone:
	...
	// check if there was an error in the subscribe job and return
	if subJobStatus == "error" {
		return nil, errors.New("fail to connect")
	}
...
// It returns a pointer to TotalPubResults, which contains the aggregated statistics of the publish results.
func calculatePublishResults(pubresults []*PubResults, totalTime time.Duration) *TotalPubResults {
	...
	// iterate through the publish results and calculate the total statistics
...
// calculateSubscribeResults calculates the total subscribe results based on the given subresults and pubresults.
// It returns a pointer to TotalSubResults, which contains the aggregated statistics of the subscribe results.
func calculateSubscribeResults(subresults []*SubResults, pubresults []*PubResults) *TotalSubResults {
	...
	// iterate through the subscribe results and calculate the total statistics
